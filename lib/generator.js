...
export async function randomBytes(count) {
    ...
    return readUrandom(count);
}

function readUrandom(count) {
    return new Promise((resolve, reject) => {
        const file = Gio.File.new_for_path('/dev/urandom');
        file.read_async(GLib_PRIORITY_DEFAULT, null, (source, openResult) => {
            let stream;
            try {
            ...
            const readMore = () => {
                stream.read_bytes_async(count - filled, GLib_PRIORITY_DEFAULT, null,
 ...
 * deterministic.
 *
 * @param {(count: number) => Promise<Uint8Array>} [read] entropy reader
 * @returns {{bytes: (count: number) => Promise<Uint8Array>}}
 */
export function bufferedSource(read = randomBytes) {
    ...
    return {
        async bytes(count) {
            if (buffer.length - offset < count) {
                buffer = await read(Math.max(count, CHUNK_SIZE));
                offset = 0;
 ...
 */
export async function generate({ length, classes, source = bufferedSource() }) {
 ...
 * @returns {Promise<string[]>}
 */
export async function generateMany({ length, classes, count, source = bufferedSource() }) {
    if (!Number.isInteger(count) || count < 1)
        throw new Error(`generateMany: invalid count ${count}`);

    const passwords = [];
    for (let i = 0; i < count; i++)
        passwords.push(await generate({ length, classes, source }));
    return passwords;
}
